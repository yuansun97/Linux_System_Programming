 ...
 *   addresses of different processes.
 * - checks to see that you are flushing the TLB, since you are context
 *   switching from different processes.
 */
int multiprocess(mmu *test_mmu) {
    ...
    // because we are going from process 1->2->1->2
...
int page_fault_no_swap(mmu *test_mmu) {
    addr32 virtual_address = 0x08052000; // virtual address of the first
                                         // DATA page.
    char buff[PAGE_SIZE];
    char *test_string = "hello world";
...
int page_fault_with_swap(mmu *test_mmu) {
    ...
    // only 1 page fault since the eviction scheme does not page page tables
    // or directories to disk.
